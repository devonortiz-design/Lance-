...
useEffect(()=>{(async()=>{const[f,s]=await Promise.all([loadMemory().catch(()=>[]),loadRecentSessions().catch(()=>[])]);setMemoryFacts(Array.isArray(f)?f:[]);setRecentSessions(Array.isArray(s)?s:[])})()},[]);
...
const speakMessage=useCallback(async(text,idx)=>{stopSpeaking();setLoadingIdx(idx);const audio=await speak(text,()=>{setLoadingIdx(null);setSpeakingIdx(idx)},()=>{setSpeakingIdx(null);audioRef.current=null},()=>{setLoadingIdx(null);setSpeakingIdx(null)});audioRef.current=audio},[stopSpeaking]);
...
if(tags.length>0){loadMemory().then(f=>{if(Array.isArray(f))setMemoryFacts(f)}).catch(()=>{})}
if(msgCount.current%4===0){const summary=`${t.slice(0,90)}${t.length>90?"…":""}`;saveSession("general",summary,msgCount.current).catch(()=>{})}
...
{isEmpty&&(<div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",animation:"fadeUp 0.35s ease both"}}><div style={{textAlign:"center",maxWidth:"360px"}}><div style={{display:"inline-block",marginBottom:"20px",animation:"pulseGlow 3s ease-in-out infinite"}}><LanceLogo size={76}/></div><div style={{fontSize:"32px",fontWeight:600,color:"#fff",letterSpacing:"-0.03em",marginBottom:"8px",textShadow:"0 2px 16px rgba(0,0,0,0.4)"}}>Good to see you, Pastor.</div><div style={{fontSize:"17px",color:"rgba(255,255,255,0.68)",fontWeight:300,lineHeight:1.55}}>Type anything, drop a file, or just talk.</div></div></div>)}
...
<button className={`speak-btn${speakingIdx===i?" active":""}`} onClick={()=>speakingIdx===i?stopSpeaking():speakMessage(m.content,i)} style={{color:speakingIdx===i?"#C9A84C":"rgba(255,255,255,0.6)"}} title={speakingIdx===i?"Stop":"Hear Lance"}><SpeakerIcon active={speakingIdx===i} spinning={loadingIdx===i}/></button>
