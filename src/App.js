...
async function ensureDocx(){if(window.docx)return;await new Promise((res,rej)=>{const s=document.createElement("script");s.src="https://unpkg.com/docx@8.5.0/build/index.umd.min.js";s.onload=res;s.onerror=rej;document.head.appendChild(s);});}
            ...
            <div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start",animation:"fadeUp 0.2s ease both",marginBottom:"4px"}}>
              ...
              <div style={{maxWidth:"72%",display:"flex",flexDirection:"column",alignItems:m.role==="user"?"flex-end":"flex-start",gap:"4px"}}>
                ...
                {m.role==="assistant"&&(
                  ...
                  <button className="speak-btn" onClick={()=>downloadDocx(m.content)} style={{color:"rgba(255,255,255,0.6)",marginLeft:"2px"}} title="Download as Word doc"><svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M6.5 1v7M3.5 5.5l3 3 3-3" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/><path d="M2 10h9" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg></button>
                )}
              </div>
            </div>
          ))}
